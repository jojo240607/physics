...
- 缺口(剩余):① GPU 数值一致性:本机(host)已补 **wgsl 等价串行参考验收**(`phy-demo-web::gpu_ref`,`cargo test -p phy-demo-web` 可跑,无 WebGPU adapter 也能验证内核数学自洽)+ `wasm-cross-check.py --gpu` 可编译守卫;真实 adapter 上的 CPU vs GPU 误差对比仍需浏览器人工目测(本环境无 WebGPU adapter)。

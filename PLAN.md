...
| **M4b 软-刚耦合** | 软体质点↔刚体(M3)碰撞双向耦合 | 软体把球推开/球压扁软体 | ✅ `SoftBody::collide_body(&Body)->Vec3` 用刚体形状 `contains_local` 检测穿透,沿外法线(6 轴支撑估算法线 + 二分求穿透深度)推出质点并反射法向速度;可动刚体按 `body_coupling` 收到反向冲量。`Subsystem` trait 加 `as_any_mut`(6 个 impl 补默认);`phy-demo::Scene::couple_soft_rigid` 每帧 `world.step` 后两阶段(快照刚体→可变软体算碰撞+冲量→可变刚体施加)规避同 World 双可变借用。`cargo test --workspace` 全过

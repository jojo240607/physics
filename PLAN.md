...
| 编号 | 缺口 | 现状 | 说明 |
|---|---|---|---|
| **G1** | 真实 GPU 逐粒子**误差报告**(CPU↔GPU 数值一致性) | ✅ 内核对齐 + host 端逐粒子误差报告(2026-08-11):W4/W5 wgsl 已改与 CPU 生产**逐公式对齐**(对称压力式 `m_i·m_j·(p_i/ρ_i²+p_j/ρ_j²)`+非牛顿幂律 `μ=ki·max(shear,shear_min)^(ni-1)`+颗粒 PBD 同款);`gpu_accuracy.rs` 现直接对比 CPU 生产逐粒子加速度 vs wgsl 串行参考,实测 SPH 逐粒子 MAX≈2.9e-6、颗粒投影逐位一致(浮点精度量级),即真实 adapter CPU↔GPU 误差的合理代理基线;真机 adapter 回填脚本(`export_flat_for_adapter` + JS/CLI 消费者)待浏览器跑通,误差量级预期与本基线一致 | 游戏要信 GPU 结果,必须有真机逐粒子 max/RMSE 报告 |
...
- 缺口(剩余):① GPU 数值一致性:本机(host)已达成 **wgsl 与 CPU 生产逐公式对齐 + 逐粒子误差基线**(`phy-demo-web::gpu_accuracy`,`cargo test -p phy-demo-web` 可跑,SPH MAX≈2.9e-6 / 颗粒投影逐位一致,即真实 adapter 浮点误差代理)+ `gpu_ref` 串行参考 + `wasm-cross-check.py --gpu` 可编译守卫;**真实 adapter 上的 CPU↔GPU 误差对比**仍需浏览器/原生 wgpu 环境跑通(本环境桌面 wgpu 链接崩溃),误差量级预期与本基线一致。

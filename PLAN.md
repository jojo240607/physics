...
| **B2** | **角色控制器 / kinematic body / sensor/trigger** | 已落地(2026-08-11):kinematic body(`Body::kinematic`,有效反质量 0,主动移动并推开动态体)+ sensor/trigger(`Body::is_sensor`,窄相参与但不施冲量,经 `RigidWorld::sensor_contacts` 报告重叠事件);**capsule 角色控制器已落地(D4,`CharacterController` + `CharacterController::update`,kinematic 胶囊 + 每帧输入驱动、撞墙自动 slide、不被碰撞推开)**,并接入 `phy-sdk`(doctest)、`phy-demo`(`Character` 模式,键盘 WASD+跳跃)与场景 DSL(`SceneDesc::character_spawn`,`load_scene_json` 一键落地)。详见 `ROADMAP_GAME.md` A1 |
...
- 缺口(剩余):① GPU 数值一致性:**已达成真机**(2026-08-11):`gpu` 模块放开 native 编译,新增 `examples/real_gpu_error.rs`,在**真实 NVIDIA Quadro P2200 adapter** 上实测 **GPU vs wgsl 串行参考逐位一致**(SPH acc MAX≈1.5e-4、颗粒投影逐位 0);真机跑修复了 wgsl 内核的格子索引错位(密度 0)与粘性缺 `m_i` 两处 bug。剩余已知限制:CPU 生产内核 vs flat 网格在边界/角落粒子的邻居查找不同(角落 SPH 力 CPU≈0 vs GPU≈36),内部粒子逐位一致,留待内核统一。
...
| P5 | GPU 数值一致性验收 | W1–W7 CPU 路由有;host 端加 `gpu_ref` wgsl 等价串行参考(`cargo test` 可跑,验证内核数学自洽)+ `wasm-cross-check.py --gpu` 可编译守卫;真实 adapter 误差对比需浏览器人工目测 | ⚠️ 真实 adapter 目测待浏览器 | 中 |
...
- [x] **P3**(2026-08-10):`phy-ffi/src/lib.rs` 新增 f32 通道入口——`phy_world_step_f32`(dt 走 f32,内部 cast f64)+ `phy_world_get_fluid_positions_f32` / `phy_world_get_fluid_velocities_f32` / `phy_world_get_rigid_transforms_f32`(写回 `f32` 缓冲,save 50% 带宽,对齐 GPU/Unity f32 世界)。内部世界恒 `World<f64>`(保 S8 回放确定性),精度在边界 cast,ABI 稳定且同一世界可混用 f32/f64 入口。新增测试 `ffi_f32_channel_matches_f64`(验证 f32 是 f64 精确 cast)/`ffi_f32_null_pointer_is_safe`。cbindgen 头与 `build_package.py` 的 def 正则自动覆盖新符号,无需手工改脚本。`cargo test -p phy-ffi` 6 用例 + 1 doc 全过。
...
- [x] **P5**(2026-08-11):GPU 数值一致性验收(host 端代理证据 + 可编译守卫)。`phy-demo-web` 新增非门控 `src/gpu_ref.rs`(不依赖 wgpu,host `cargo test` 可跑),把 W4 `SPH_WGSL`(poly6/spiky_grad/visc_lap + density/force)与 W5 `GRANULAR_WGSL`(contact + apply 含 ×SCALE→÷SCALE 定点路径)的浮点数学逐行复刻为确定性串行 Rust。4 个验收测试:`cpu_ref_sph_lattice_is_reasonable`(静止晶格 mean_rho/rest∈[0.85,1.15]、全 finite)、`cpu_ref_sph_outputs_finite_on_random_cloud`(随机云无 NaN)、`cpu_ref_granular_resolves_overlap`(重叠球投影后 dist≥min_d-1e-2)、`cpu_ref_granular_clamps_to_bounds`(越界粒子夹回)。Cargo.toml 把 `phy-fluid`/`phy-granular` 从 gpu-feature optional 改为始终依赖(供 gpu_ref 引用 FlatData 类型,二者纯 CPU 不拉 wgpu)。`lib.rs` 无条件 `pub mod gpu_ref;`。注:W4/W5 wgsl 是 CPU 生产实现的**简化移植**(压力力系数 `(p_i+p_j)/(2ρ_j)`、粘性 `(k+shear_min)` 线性,与 CPU 幂律/对称式不同),故 gpu_ref 对照的是 wgsl 自身契约(finite / 密度守恒 / 不穿透),而非与 CPU 生产逐位相等;真实 adapter 上的 CPU-vs-GPU 误差对比仍待浏览器目测(本环境无 WebGPU adapter)。CHANGELOG 已知限制同步更新。
- [x] **G1**(2026-08-11):在 P5 基础上把 W4/W5 wgsl 内核**升级为与 CPU 生产逐公式对齐**(对称压力式 `m_i·m_j·(p_i/ρ_i²+p_j/ρ_j²)` + 非牛顿幂律 `μ=ki·max(shear,shear_min)^(ni-1)` + 颗粒 PBD 投影同款),`gpu_accuracy.rs` 同步把"二阶差分近似对比"改为"CPU 生产逐粒子加速度 vs wgsl 串行参考"直接对比;实测 SPH 逐粒子 MAX≈2.9e-6、颗粒投影逐位一致(浮点精度量级),即真实 adapter CPU↔GPU 误差的合理代理基线。验收测试收紧为一致性阈值(SPH MAX<1e-1、颗粒投影<1e-3)。
- [x] **G1 真机**(2026-08-11,后续):本机安装 MSVC 工具链 + VS Build Tools 后,`gpu` 模块从 `wasm32-only` 放开到 `#[cfg(feature="gpu")]`(native 也编译,wgpu 原生后端),并为原生回读补 `device.poll(PollType::Wait)`.新增 `examples/real_gpu_error.rs`,在真实 **NVIDIA Quadro P2200** adapter 上跑 W4/W5 内核.真机实测发现**两个 wgsl 内核 bug**:①格子索引 `cell_idx` 用 `ci=floor((pi-gmin)/h)`(相对偏移)再减 `mi`,双倍偏移 → SPH 密度全 0 只剩重力;修 `floor(pi/h)` 绝对 key(与 flat 网格 `flat_idx` 一致).②粘性力缺 `m_i` 因子(与 CPU 生产 `compute_forces` 不一致);补上.同时压力方向修正为 `(pi-pj)/r × 正 fpress`(推开,与 CPU 生产一致).修复后真机报告:GPU vs wgsl 参考 **SPH acc MAX≈1.5e-4、颗粒投影逐位 0**(浮点量级).已知限制:CPU 生产 `for_each_neighbor`(BTreeMap)与 flat 网格(前缀和)在边界/角落粒子的邻居查找不同(角落 SPH 力 CPU≈0 vs GPU≈36,GPU 更合理),内部粒子逐位一致,留待 phy-fluid 内核统一.
- [x] **P6**(2026-08-11):`cargo build --workspace`(含 release)零 Rust 代码警告。清理 phy-optics(`math/renderer/scene/subsystem/caustics` 未用 `na`/`FromPrimitive`/`Surface` 导入、冗余 `two`/`eps` 变量、多余括号)、phy-solid(`fem.rs` 前缀 `_cxy/_cyz/_czx`、`mesh.rs` 对 `tip_displacement`/`apply_tip_load`/`tet` 加 `#[allow(dead_code)]`)、phy-soft(`cloth.rs` 移除闭包多余 `mut`)、phy-fluid(`sph/grid.rs` 对 `FlatGrid.cell` 加 `#[allow(dead_code)]`)、phy-demo(`scene.rs` 把 `Joint` 移入 `#[cfg(test)]`、移除 `heat_sub`/`rsub` 多余 `mut`)、phy-rigid(`world.rs` 移除测试 `rho` 多余 `mut`)。仅剩 MinGW 链接器 `corrupt .drectve` 提示(rust-lang/rust#50633 工具链缺陷,非代码),crate 顶加 `#![allow(linker_messages)]` 静音。
- [x] **P7**(2026-08-11):`phy-ffi/src/lib.rs` 头部新增 ABI 稳定性契约——`PHY_FFI_ABI_VERSION`(u32=1,单调递增)、`PHY_FFI_VERSION_{MAJOR,MINOR,PATCH}`(0.1.0)常量 + `#[no_mangle] pub extern "C" fn phy_ffi_abi_version()` 运行时查询,破坏任一 `phy_*` 符号需自增并在 CHANGELOG "Changed" 记录。CHANGELOG 新增 "ABI stability policy" 段。cbindgen 头 `phy_ffi.h` 自动带 `#define PHY_FFI_ABI_VERSION 1` 与 `phy_ffi_abi_version()` 声明。`cargo test -p phy-ffi` + 全 workspace 测试通过。提交 `72e5358`。

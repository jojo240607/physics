...
## 五、超越商用路线图（Beyond-Commercial）

目标不是"媲美商用"，而是在商用引擎的**共性弱点**上建立代差级优势。详细计划见
[`docs/BEYOND_COMMERCIAL.md`](./BEYOND_COMMERCIAL.md)。核心差异化 = **统一多物理场确定性引擎**
（一个 `World` 同步演化刚/流/软/热/光/电磁/引力，自动双向耦合），商用单场工具箱无法表达。

- **A1 统一耦合闭环 demo ✅**：`examples/coupled_world.rs` 已验证刚/流/软/光四场在同一 `World`
  内自动双向耦合（落球浮力托起 + 软体被流场推动 + 光学实时焦散），无手写胶水。
- **B0 [关键攻关] SPH 入水冲击数值刚性**：A1 实测暴露 SPH 耦合能量瞬态暴涨（见
  BEYOND_COMMERCIAL.md B 阶段），是超越商用必须攻克的科学问题——我们的统一架构让问题
  **可量化、可定位**（逐场能量审计），这是商用引擎不具备的观测能力。
- 后续：A2 多场确定性交叉验证 → B 系列规模化/稳定性 → C 工业级工具链 → D 杀手级差异化
  （物理驱动焦散全局光照 / 确定性云同步原语 / 自定义物理场 SDK）。

...
pub struct OpticSubsystem<T: RealField + Copy> {
    /// 场景。
...
impl<T: RealField + Copy> OpticSubsystem<T> {
    /// 构造。
    pub fn new(scene: OpticScene<T>, precision: Precision) -> Self {
        Self { scene, precision }
    }
...
impl<T: RealField + Copy> Subsystem<T> for OpticSubsystem<T> {
